...
#radii_df.to_csv("results/snapshot_2_radii_projected_fix.csv",index=False)

#sled_df = pd.DataFrame({"Galaxy_ID":[],"sled_CO10":[],"sled_CO21":[],"sled_CO32":[],"sled_CO43":[],"sled_CO54":[],"sled_CO65":[],"sled_CO76":[],"sled_CO87":[],"sled_CO98":[],})
#sled_df.to_csv("results/sled_snapshot_2_fix.csv",index=False)
...
for cloud in clouds_in_each_galaxy[args.galinfoline-1:args.galinfoline]:
    ...
    clean_pct = float(len(clean_list)/len(obj.galaxies[gal_id].glist)*100)
    ...
    df_aux = df_aux.sort_values(by='Radius')
...
radii_df.to_csv("results/snapshot_2_radii_projected_fix.csv",index=False,mode='a',header=False)
...
#sled_df.to_csv("results/sled_snapshot_2_fix.csv",index=False,mode='a',header=False)
