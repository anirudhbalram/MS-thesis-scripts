...
def create_basic_table(config):
    ...
    clouds_in_each_galaxy = [(gal.GroupID,gal.glist,gal.masses['gas'].in_units('Msun').value,gal.masses['H2'].in_units('Msun').value,gal.radii['baryon_half_mass'].in_units('pc').value,gal.metallicities['mass_weighted'].value) for gal in [obj.galaxies[int(GroupID)] for GroupID in selected_ids_array]]
    ...
    basic_filename = f'{config["output_dir"]}/Basic_Characteristics_z_0.8_MS.csv'
    ...
    output_df.to_csv(f'{config["output_dir"]}/lim_df_z_0.8_MS.csv', index = False)    #creating the output file whose contents are later appended in limfunctions
