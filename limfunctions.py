...
def submm_luminosity(INPUT, NZONES=25, ProfileType = 'Powerlaw',noClump = False):
    ...
    conv_bool = gmc.setChemEq(network=GOW, evolveTemp = 'iterateDust', verbose=False, info = {'xC': 1.6e-4*Metallicity,'xO':3.2e-4*Metallicity,'xSi':1.7e-6*Metallicity}, tol = 1.e-3, maxTime=1e20)
...
def creating_table(cloud_list, df_basic, output_dir):
    ...
    for c in cloud_list:
        ...
        try:
            ...
            conv_flag = sum([(int(conv_digit)-1) for conv_digit in out[39]])
            #if out[39]==True:
            #    conv_flag = 0
            #else:
            #    conv_flag = 1
    ...
    df.to_csv(f'{output_dir}/lim_df_z_0.8_MS.csv', index = False, mode='a', header=False)
