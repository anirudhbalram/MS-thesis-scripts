...
muH_atom = 1.4    #atomic weight (mean mass per H nuclei, equivalent to gmc.muH in despotic for xH2 = 0.5 and xHe = 0.1)
...
#to compute densities when despotic fails

def except_compute_temp_dens(Mcloud,Rcloud,Tg,Td,NZONES):
    ...
    gas_temp = -99
    dust_temp = -99
    n_dens = np.median(density.value)
...
def submm_luminosity(INPUT, NZONES=25, ProfileType = 'Powerlaw',noClump = False):
    ...
    if ProfileType != 'Fixed':
        ...
        density /= muH_atom #attomic weight correction
    ...
    gmc.sigmaD10   = 2.0e-25  * Metallicity * DMR       # Cross section to 10K thermal radiation, cm^2 H^-1 (check Despotic paper for values)
    ...
    #conv_bool = gmc.setChemEq(network=NL99_GC, evolveTemp = 'iterateDust', verbose=True, info = {'xC': 2.e-4*Metallicity,'xO':4.e-4*Metallicity}, tol = 1e-3, maxTime = 1e17)
    
    conv_bool = gmc.setChemEq(network=GOW, evolveTemp = 'iterateDust', verbose=False, info = {'xC': 1.6e-4*Metallicity,'xO':3.2e-4*Metallicity,'xSi':1.7e-6*Metallicity}, tol = 1.e-3, maxTime=1e20)    #tol is the tolerance to compute the convergence. maxTime is the total time (equilibria time for the cloud, not CPU/real time) allowed for the cloud to try to converge
    
    conv_str= ''.join([str(int(conv)+1) for conv in conv_bool])    #string of 1s and 2s (1 means not converged, 2 means converged)
    ...
    n_dens,col_dens,gas_temp,dust_temp,sigmaNT = np.median(gmc.nH),np.median(column_density_cgs.value),np.median(gmc.Tg),np.median(gmc.Td),np.median(gmc.sigmaNT)
...
def creating_table(cloud_list, df_basic, output_dir):
    ...
    for c in cloud_list:
        ...
        if tnow-tstart>tdelta:    #if runtime has crossed 95 hours, it stops the script and saves existing output (this is done to avoid wasting computing time as the HiPerGator burst queue time limit is 96 hours - jobs will be abruptly stopped after that and data would be lost)
            break
        ...
        SFR = float(df_basic['c_SFR'][df_basic['c_Index']==c])
        coord_x = float(df_basic['x'][df_basic['c_Index']==c])
        coord_y = float(df_basic['y'][df_basic['c_Index']==c])
        coord_z = float(df_basic['z'][df_basic['c_Index']==c])
        ...
        try:
            ...
            conv_flag = sum([(int(conv_digit)-1) for conv_digit in out[39]])    #flag to check how many zones have converged in 16
            df_aux = pd.DataFrame({'Galaxy_ID':int(gal_id), 'Cloud_ID':int(c), 'Mcloud':out[0], 'Rcloud':out[1], 'c_Pressure':Pressure, 'Metallicity':out[2], 'RadField':out[3], 'c_DMR':DMR, 'c_SFR':SFR, 'redshift':out[4], 'H2_lcii':out[5], 'CO10':out[6], 'CO21':out[7], 'CO32':out[8], 'CO43':out[9], 'CO54':out[10], 'CO10_intTB':out[11], 'CO21_intTB':out[12], 'CO32_intTB':out[13], 'CO43_intTB':out[14], 'CO54_intTB':out[15], 'CI10':out[16], 'CI21':out[17], 'CO65':out[18], 'CO76':out[19], 'CO87':out[20], 'CO98':out[21], 'CO65_intTB':out[22], 'CO76_intTB':out[23], 'CO87_intTB':out[24], 'CO98_intTB':out[25], 'OI1':out[26], 'OI2':out[27], 'OI3':out[28], 'fH2':out[29], 'fH':out[30], 'fHp':out[31], 'gas_temp':out[32], 'dust_temp':out[33], 'n_dens':out[34], 'col_dens':out[35], 'sigmaNT':out[36], 'Mol_gas':out[37], 'CO10_areal_TB':out[38], 'x':coord_x, 'y':coord_y, 'z':coord_z, 'conv_flag':conv_flag, 'conv_str':out[39], 'time':time},index=[1])
            df = pd.concat([df,df_aux])
        except:
            t2 = date.now()
            time = (t2-t1).total_seconds()
            conv_fail = '1111111111111111'
            conv_flag = 0
            gas_temp,dust_temp,n_dens,col_dens = except_compute_temp_dens(Mcloud*u.Msun,Rcloud*u.pc,Tg=-99*np.ones(16),Td=-99*np.ones(16),NZONES=16)
            df_aux = pd.DataFrame({'Galaxy_ID':int(gal_id), 'Cloud_ID':int(c), 'Mcloud':Mcloud, 'Rcloud':Rcloud, 'c_Pressure':Pressure, 'Metallicity':Metallicity, 'RadField':RadField, 'c_DMR':DMR, 'c_SFR':SFR, 'redshift':redshift, 'H2_lcii':-99, 'CO10':-99, 'CO21':-99, 'CO32':-99, 'CO43':-99, 'CO54':-99, 'CO10_intTB':-99, 'CO21_intTB':-99, 'CO32_intTB':-99, 'CO43_intTB':-99, 'CO54_intTB':-99, 'CI10':-99, 'CI21':-99, 'CO65':-99, 'CO76':-99, 'CO87':-99, 'CO98':-99, 'CO65_intTB':-99, 'CO76_intTB':-99, 'CO87_intTB':-99, 'CO98_intTB':-99, 'OI1':-99, 'OI2':-99, 'OI3':-99, 'fH2':-99, 'fH':-99, 'fHp':-99, 'gas_temp':gas_temp, 'dust_temp':dust_temp, 'n_dens':n_dens, 'col_dens':col_dens, 'sigmaNT':-99, 'Mol_gas':-99, 'CO10_areal_TB':-99, 'x':coord_x, 'y':coord_y, 'z':coord_z, 'conv_flag':conv_flag, 'conv_str':conv_fail, 'time':time},index=[0])
            df = pd.concat([df,df_aux])
            pass
    ...
    df.to_csv(f'{output_dir}/lim_df_z_2_MS.csv', index = False, mode='a', header=False)    #this file can be analysed and used for all post-processing scripts. It contains luminosities and other output quantities.
